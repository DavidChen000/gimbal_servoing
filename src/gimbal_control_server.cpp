...
#include "amov_gimbal_sdk_ros/gimbal_control.h"
...
bool commandCallback(amov_gimbal_sdk_ros::gimbal_control::Request &req, amov_gimbal_sdk_ros::gimbal_control::Response &res ){
    gimbal_control_.header = req.header;
    gimbal_control_.mode = req.mode;

    gimbal_control_.roll_angle = req.roll_angle;
    gimbal_control_.pitch_angle = req.pitch_angle;
    gimbal_control_.yaw_angle = req.yaw_angle;

    gimbal_control_.roll_rate = req.roll_rate;
    gimbal_control_.pitch_rate = req.pitch_rate;
    gimbal_control_.yaw_rate = req.yaw_rate;

    res.success = true;
    res.message = "Controling the gimbal to move !";
...
int main(int argc, char *argv[]) {
  ...
  ros::init(argc, argv, "gimbal_control_server");//initialize rosnode "gimbal_control_server"
  ros::NodeHandle nh("~");
  ...
  ros::ServiceServer gimbal_service = nh.advertiseService("gimbal_control_command",commandCallback);
  //Create a rosservice named "gimbal_control_command",register the callback function "commandCallback"

  ROS_INFO("Ready to recevice gimbal control command.");



//   ros::Subscriber sub = nh.subscribe<amov_gimbal_sdk_ros::GimbalControl>(
//       "/amov_gimbal_ros/gimbal_control", 1000, control_callback);
// rosnode for gimbal from amov
